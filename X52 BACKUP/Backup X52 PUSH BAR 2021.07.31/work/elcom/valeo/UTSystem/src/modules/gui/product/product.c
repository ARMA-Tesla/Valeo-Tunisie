...
static SLabelListPtr		gs_pLabels;
...
static SElExceptionPtr Read_Config(const char*   fileName, int *Use_Barcode);
      ...
      void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
  		...
  		case EVENT_COMMIT:
			EXCCHECKCVI( GetCtrlIndex (panel, control, &index));
			EXCCHECKCVI( GetValueFromIndex (panel, control, index, &product_id));
			EXCCHECKCVI( GetLabelFromIndex (panel, control, index, Ref));  
		    EXCCHECKCVI( GetNumListItems (panel, control, &NumOfItems));
...
//	EXCCHECKCVI( SetPanelAttribute (gs_panel[PRODUCT], ATTR_TOP, top + height - PANEL_TOP));
//	EXCCHECKCVI( SetPanelAttribute (gs_panel[PRODUCT], ATTR_LEFT, PANEL_LEFT));
	EXCCHECKCVI( SetPanelAttribute (gs_panel[PRODUCT], ATTR_TOP, top ));
	EXCCHECKCVI( SetPanelAttribute (gs_panel[PRODUCT], ATTR_LEFT, 0));
	EXCCHECKCVI( SetPanelAttribute (gs_panel[PRODUCT], ATTR_HEIGHT, height-78));
	EXCCHECKCVI( SetPanelAttribute (gs_panel[PRODUCT], ATTR_WIDTH, width));
...
void CVICALLBACK ReadFunction (int portNumber, int eventMask, void *callbackData)
{
	...
	/* lecture label Z4 */
	if (ChaineLue[Index+1] == 'H')
	{
	for(icmpt=Index+1;icmpt<Lenght;icmpt++)
			if(ChaineLue[icmpt]==32) break;
	
	CopyString (RefLue, 0, ChaineLue, Index+2, icmpt-2);
		
	EXCCHECK( gs_pProductList->ProductCheckRef(gs_pProductList, RefLue, &ProdcutId));

	EXCCHECK( ChildExist( gs_pProductList,ProdcutId, &child, &parent_id));
	
		if (!child && parent_id)
		{	
		EXCCHECK(  ProductSelected(ProdcutId, FALSE)); 
		CopyString (ChaineLue, 0, ChaineLue, Index+1, -1);
   		ComWrt (PORTCIM, ChaineLue, StringLength (ChaineLue));
		Sleep(200);
		ReturnRS232Err ();
		Sleep(200);
		}
