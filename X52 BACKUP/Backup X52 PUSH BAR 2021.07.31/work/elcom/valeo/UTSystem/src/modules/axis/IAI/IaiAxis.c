...
void LogAxis( char *Error)   
{
	...
	sprintf(result_dir, "%s\\LOG_AXIS\\%04d.%02d.%02d", project_dir, st.wYear, st.wMonth, st.wDay);
	CheckDirectory(result_dir); 

	
	/* enreg des data */
	memset(log, 0x00, 256);
	sprintf (fileName, "%s/Log_axis.txt", result_dir);
...
sprintf(log_PresentAlarm,"Present Alarm ==> Axe N: %d ; Read_Buffer : %s",axis, read_buffer);
LogAxis(log_PresentAlarm);   
	if (read_buffer)
	{
...
command[17] = '0';
command[18] = '0';
if (position > 0.0 && position < 300.0 )
