...
static SElExceptionPtr master_ProductMasterSave(struct _SDBSMasterList* me, int32_t branch_id)
{
	...
	if(me->ProductMasterSize)
	{
		...
		/* get last test_id */
		EXCCHECK( podbc->ExecSQL( podbc, "SELECT MAX(pm.parameter_id) FROM valeo.product_masters pm"));
		CHECKERR( podbc->Fetch( podbc, TRUE));
		...
		{	
			...
			sprintf(sql, "REPLACE INTO valeo.product_masters "
...
static SElExceptionPtr master_ProductMasterDelHist(struct _SDBSMasterList* me, int32_t branch_id)
{
	...
	sprintf(sql, "DELETE FROM valeo.product_masters "
...
)
{
	...
	*master_id = 0;
	...
	do
	{
		//printf("ProductMasterSize=%d\r\n", me->ProductMasterSize);
		//printf("product_id=%d\r\n", product_id);
		//printf("master_code=%s\r\n", master_code);
		//printf("strlen(master_code)=%d\r\n", strlen(master_code));
		//printf("---------------------------------------------------\r\n");
		//Delay (4);
		//ProcessSystemEvents ();
		//ProcessDrawEvents ();
		for(i=0; i<me->ProductMasterSize; i++)
		{
			//printf("me->ProductMaster[i].product_id=%d\r\n", me->ProductMaster[i].product_id);
			//printf("me->ProductMaster[i].master=%s\r\n", me->ProductMaster[i].master);
			//printf("strlen(me->ProductMaster[i].master)=%d\r\n", strlen(me->ProductMaster[i].master));
			//printf("me->ProductMaster[i].property&PROPERTY_VALID=%d\r\n", me->ProductMaster[i].property&PROPERTY_VALID);
			//printf("---------------------------------------------------\r\n");
			if( (me->ProductMaster[i].property&PROPERTY_VALID)>0
			 	&& me->ProductMaster[i].product_id == product_id 
				&& strcmp(master_code, me->ProductMaster[i].master)==0)
			{	
				printf("me->ProductMaster[i].product_id=%d\r\n", me->ProductMaster[i].product_id);
				run = FALSE;
