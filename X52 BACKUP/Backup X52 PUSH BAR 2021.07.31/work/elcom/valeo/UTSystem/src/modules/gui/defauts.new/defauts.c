					...
					do{
						...
						if(ptext && *ptext)
							...
							if (strcmp(ptext, "1") == 0) /* mesure Failed */
								{
						    	sprintf(path, "test[@step='%d']/measure[@step='%d']/limit/id", i,j);  /* Get ID of Mesure Failed */
				    			...
				    			pparameter4 = 0;	
						   		...
						   		sprintf(path, "test[@step='%d']/measure[@step='%d']/config", i,j); /* Get config of Mesure Failed */ 
