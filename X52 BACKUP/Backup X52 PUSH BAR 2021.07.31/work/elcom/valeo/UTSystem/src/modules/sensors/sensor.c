...
#define ADDRESS_CPX 	5

#define NUMBER_MODULE_DI 	 	 3 
#define BYTES_PER_MODULE_DI 	 4 
#define FIRST_MODULE_POSITION_DI 2 

#define NUMBER_MODULE_D0 	 	 1 
#define BYTES_PER_MODULE_DO 	 4 
#define FIRST_MODULE_POSITION_DO 1

#define NUMBER_MODULE_CPV 	 	  4 
#define BYTES_PER_MODULE_CPV 	  8
#define FIRST_MODULE_POSITION_CPV 1 
			...
			if(pitem)
			{
				...
				if(pparameter)
				{
					...
					if(ptext)
					{
						...
						else if( 0==strcmp(ptext, "CPV"))
						{
							pTask_config->type = CPV_DO;
						}
...
)
{
	...
	EXCCHECKCVI (system ("C:\\Program Files\\BradCommunications\\applicomIO\\4.2\\PCINITIO.exe"));
...
static DWORD WINAPI Thread_Sensor(LPVOID param)
{
	...
	while(TRUE)
	{
		...
		if (WaitForSingleObject(pSensors->_hEventStop, 5) == WAIT_OBJECT_0)
		{
			for(i=0; i<pConfig->TaskCount; i++)
			{
				free( pold_data[i]);
	...
	//	is_locked = TRUE;
		...
		if ( pConfig->IsSimulation)
		{
			...
			strcpy( tmp.name, "PLC_MOD2_P0_DI");
			...
			strcpy( tmp.name, "PLC_MOD2_P1_DI");
			ptask_config = bsearch( &tmp, pConfig->pTasks, pConfig->TaskCount, sizeof(STaskConfig), CompareTasks);	
			if( ptask_config)
			{
				ptask_config->value = 0;
			...
			strcpy( tmp.name, "PLC_MOD2_P2_DI");
			ptask_config = bsearch( &tmp, pConfig->pTasks, pConfig->TaskCount, sizeof(STaskConfig), CompareTasks);	
			if( ptask_config)
			{
				ptask_config->value = 0;
			...
			strcpy( tmp.name, "PLC_MOD2_P3_DI");
			ptask_config = bsearch( &tmp, pConfig->pTasks, pConfig->TaskCount, sizeof(STaskConfig), CompareTasks);	
			if( ptask_config)
			{
				ptask_config->value = 0;
			...
			strcpy( tmp.name, "CPV_MOD1_P1_DO");
			ptask_config = bsearch( &tmp, pConfig->pTasks, pConfig->TaskCount, sizeof(STaskConfig), CompareTasks);	
			if( ptask_config)
			{
				ptask_config->value = 0;
			...
			strcpy( tmp.name, "CPV_MOD1_P2_DO");
			ptask_config = bsearch( &tmp, pConfig->pTasks, pConfig->TaskCount, sizeof(STaskConfig), CompareTasks);	
			if( ptask_config)
			{
				ptask_config->value = 0;
			...
			strcpy( tmp.name, "CPV_MOD1_P3_DO");
			ptask_config = bsearch( &tmp, pConfig->pTasks, pConfig->TaskCount, sizeof(STaskConfig), CompareTasks);	
			if( ptask_config)
			{
				ptask_config->value = 0;
			...
			strcpy( tmp.name, "CPV_MOD1_P4_DO");
			ptask_config = bsearch( &tmp, pConfig->pTasks, pConfig->TaskCount, sizeof(STaskConfig), CompareTasks);	
			if( ptask_config)
			{
				ptask_config->value = 0;
			...
			strcpy( tmp.name, "CPV_MOD1_P5_DO");
			ptask_config = bsearch( &tmp, pConfig->pTasks, pConfig->TaskCount, sizeof(STaskConfig), CompareTasks);	
			if( ptask_config)
			{
				ptask_config->value = 0;
		...
		{   
			...
			BYTE TabWriteByteCPV [NUMBER_MODULE_CPV * BYTES_PER_MODULE_CPV]; /* CPV D0 6 Bytes */
			int Bit;
			...
			if (i > 0)
				...
				else if( pConfig->pTasks[plogic_names->TaskIndex].type == CPV_DO)
				{
					for( j=0; j<pConfig->pTasks[plogic_names->TaskIndex].count; j++)
					{   
						if (j > 0)
						...
						if (j==0)
						{
							sscanf(plogic_names->physical, "MOD%d/EV%d.%d", &Module, &Port, &Line);
							TabWriteByteCPV[((Module-FIRST_MODULE_POSITION_CPV)*BYTES_PER_MODULE_CPV)+Port] = pConfig->pTasks[plogic_names->TaskIndex].values[j] * pow(2,j);
						}	
						else
						{
						sscanf(plogic_names->physical, "MOD%d/EV%d.%d", &Module, &Port, &Line);
						TabWriteByteCPV[((Module-FIRST_MODULE_POSITION_CPV)*BYTES_PER_MODULE_CPV)+Port] = TabWriteByteCPV[((Module-FIRST_MODULE_POSITION_CPV)*BYTES_PER_MODULE_CPV)+Port] + (pConfig->pTasks[plogic_names->TaskIndex].values[j] * pow(2,j));
						pConfig->pTasks[plogic_names->TaskIndex].value = (uint32_t)TabWriteByteCPV[((Module-FIRST_MODULE_POSITION_CPV)*BYTES_PER_MODULE_CPV)+Port];
						}
...
//						{
//							TabWriteByteCPV[(Port-1)/4] = pConfig->pTasks[plogic_names->TaskIndex].values[j] * pow(2,j);
//							Initialize_Byte = 1;
//						}	
//						else
//						{
//						TabWriteByteCPV[(Port-1)/4] = TabWriteByteCPV[(Port-1)/4] + (pConfig->pTasks[plogic_names->TaskIndex].values[j] * pow(2,j));
//						pConfig->pTasks[plogic_names->TaskIndex].value = (uint32_t)TabWriteByteCPV[(Port-1)/4];
//						}
					}
			...
			EXCCHECKCPLC( IO_WriteQByte (CARTE_PROFIBUS, ADDRESS_CPX, 0, NUMBER_MODULE_CPV * BYTES_PER_MODULE_CPV, TabWriteByteCPV, &Status));
