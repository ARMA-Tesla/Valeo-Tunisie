...
void LogPrinterSensorError( char *Error)   
{
	...
	sprintf(result_dir, "%s\\printerSensors\\%04d.%02d.%02d", project_dir, st.wYear, st.wMonth, st.wDay);
	CheckDirectory(result_dir); 

	
	/* enreg des data */
	memset(log, 0x00, 256);
	sprintf (fileName, "%s/Log_PrinterSensor.txt", result_dir);
...
if (microCam && (status_rodage_x52 || status_rodage_vs11))
	{	
		...
		for (i=0;i<cycle;i++)
		{
			/*************** Rodage all pushs at the same time ***************/
...
)
{
	...
	{	
		...
		EXCCHECK( me->GetSensor(me, "MICRO_CAM", &microCam));
		//printf ("UnlockPanelStation10 Tester.c , microCam = %d \r \n",microCam);
		Sleep(20);
		}
...
)
{
	...
	if(microCam && sensor_printer_sensor_rear)
	...
	else 
		{
			...
			sprintf(log,"Tester.c => CyclePrint => TESTER_ERROR_CYCLE_PRINTER :  microCam = %d , sensor_printer_sensor_rear = %d \r \n",microCam,sensor_printer_sensor_rear);
			LogPrinterSensorError(log);
...
Error:
	...
	if(pexception)
	{
		sprintf(log,"Tester.c => CyclePrint => pexception !=NULL :  is_cylinder_printer_rear = %d , sensor_printer_sensor_rear = %d \r \n ",is_cylinder_printer_rear,sensor_printer_sensor_rear);
		LogPrinterSensorError(log);	
	}
...
)
{
	...
	EXCCHECK( me->axissAPI->GetaxisByIndex(me->axissAPI, 5, &paxis));
	EXCCHECK( paxis->PresentAlarm(paxis, AXIS_Z, &status));
	if (status & 0X0080 == 128)
		{
