...
static SElExceptionPtr product_ProductRename(struct _SDBSProductList* me, int32_t branch_id, int32_t product_id, char* name);
...
static SElExceptionPtr product_ProductCheckRef(struct _SDBSProductList* me, char* ProductPartNumber, int *product_id);
...
SElExceptionPtr dbsproductlist_new(SDBSProductListPtr* pDBSProductListPtr, void* pDBS)
{
	...
	me->ProductRename	= product_ProductRename;
	...
	me->ProductCheckRef	= product_ProductCheckRef;
...
static SElExceptionPtr product_ProductSave(struct _SDBSProductList* me, int32_t branch_id)
{
	...
	/* lock database valeo.test_steps */
	PDBS_LOCK(LOCK_STR);
	...
	{	
		...
		/* get last test_id */
		EXCCHECK( podbc->ExecSQL( podbc, "SELECT MAX(pt.parameter_id) FROM valeo.product_types pt"));
		CHECKERR( podbc->Fetch( podbc, TRUE));
		...
		{	
			...
			sprintf(sql, "REPLACE INTO valeo.product_types "
						 "(product_id,name,description,time,user_id,property,parameter_id) "
						 "VALUES (%d,'%s',_latin2'%s','%s',%d,%d,%d)", 
			...
			for(; i<me->ProductSize; i++)
			{
				...
				sprintf(sql, ",(%d,'%s',_latin2'%s','%s',%d,%d,%d)", 
							 me->Product[i].product_id,
							 me->Product[i].name, 
							 GET_STRING(me->Product[i].description),
							 me->Product[i].time,
...
/*---------------------------------------------------------------------------*/ 
#undef __FUNC__
#define __FUNC__ "product_ProductRename"
static SElExceptionPtr product_ProductRename(struct _SDBSProductList* me, int32_t branch_id, int32_t product_id, char* name)
{
	int32_t			error = 0;
	SElExceptionPtr	pexception = NULL;
	...
	{	
		...
		{	
			...
			if (me->Product[i].product_id == product_id)
			{
			sprintf(sql, "REPLACE INTO valeo.product_types "
						 "(product_id,name,description,time,user_id,property,parameter_id) "
						 "VALUES (%d,'%s',_latin2'%s','%s',%d,%d,%d)", 
...
)
{
	...
	sprintf(sql, "SELECT MAX(product_id) FROM valeo.product");
	EXCCHECK( podbc->ExecSQL( podbc, sql));
	CHECKERR( podbc->Fetch( podbc, TRUE));
	...
	sprintf(sql, "INSERT INTO valeo.product "
...
)
{
	...
	for(i=0; i<pidSize; i++)
	{
		sprintf(sql, "SELECT pc.product_id FROM valeo.product_counter pc "
		...
		{
			sprintf(sql, "REPLACE INTO valeo.product_counter "
...
/*---------------------------------------------------------------------------*/
#undef __FUNC__
#define __FUNC__ "product_ProductCheckRef"
static SElExceptionPtr product_ProductCheckRef(
	struct _SDBSProductList* me,
...
)
{
	...
	*product_id = 0;
	LOCK;
		...
		sprintf(sql, "SELECT product_id,property FROM valeo.product_types where name like '%s'", ProductPartNumber);
		EXCCHECK( podbc->ExecSQL( podbc, sql));
		CHECKERR( podbc->Fetch( podbc, TRUE));
		EXCCHECK( podbc->GetRows(podbc, &count));
		for(i=0; i<count; i++)
		{
		property = *((int*)podbc->GetFieldValuePtr(podbc, 2));
		if((property&PROPERTY_VALID)>0) 
			{
			*product_id	= *((int*)podbc->GetFieldValuePtr(podbc, 1));
	...
	do
	{
		for(i=0; i<me->ProductSize; i++)
		{
				...
				&& me->Product[i].parent_id == pid[index])
			{
				...
				{	sprintf( Message,"%d vs %d",allocated_size,pidIndex);
				//	EXCTHROW(-1, "Not enough allocated space!");
					EXCTHROW(-1, Message); }
