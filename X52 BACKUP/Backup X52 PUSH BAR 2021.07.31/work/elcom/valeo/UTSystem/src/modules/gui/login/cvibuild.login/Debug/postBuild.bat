@set CVIDIR=c:\program files\national instruments\cvi2015
@set CVISHAREDDIR=C:\Program Files\National Instruments\Shared\CVI
@set CVIPUBDOCSDIR=C:\Users\Public\Documents\National Instruments\CVI
@set CVIPROJPATH=c:\work\elcom\valeo\UTSystem\src\modules\gui\login\login.prj
@set CVIPROJDIR=c:\work\elcom\valeo\UTSystem\src\modules\gui\login
@set CVIPROJNAME=login.prj
@set CVITARGETPATH=c:\FlexiTester.OK\plugins\login.dll
@set CVITARGETDIR=c:\FlexiTester.OK\plugins
@set CVITARGETNAME=login.dll
@set CVIBUILDCONFIG=Debug
@set CVITARGETFILEVER=1.0.0.0
@set CVITARGETPRODVER=1.0.0.0
@set CVIVXIPNPDIR=C:\Program Files\IVI Foundation\VISA\winnt
@set CVIIVIDIR=C:\Program Files\IVI Foundation\IVI
@set CVIWINDIR=C:\Windows
@set CVISYSDIR=C:\Windows\system32
@pushd "c:\work\elcom\valeo\UTSystem\src\modules\gui\login"
xcopy /y %CVIPROJDIR%\login.xml  c:\work\elcom\valeo\UTSystem\src\app\plugins\
