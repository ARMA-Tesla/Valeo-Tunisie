...
static SElExceptionPtr labels_GetBarCodeHousingStatus(
	struct _SLabelList*	me,
	const char*			barCode, 
	bool_t*				tested,
	bool_t*				failed
);
static SElExceptionPtr labels_GetBarCodeTopCoverStatus(
	struct _SLabelList*	me,
	const char*			barCode, 
	bool_t*				tested,
	bool_t*				failed
...
)
{
	...
	plabels->GetBarCodeHousingStatus	= labels_GetBarCodeHousingStatus;
	plabels->GetBarCodeTopCoverStatus	= labels_GetBarCodeTopCoverStatus;
...
//	printf("buffer=%s  | strlen(buffer)=%d | barCode=%s  | strlen(barCode)=%d\r\n\r\n",buffer, strlen(buffer), barCode, strlen(barCode));
...
/*---------------------------------------------------------------------------*/
#undef  __FUNC__
#define __FUNC__	"labels_GetBarCodeHousingStatus"
static SElExceptionPtr labels_GetBarCodeHousingStatus(
	struct _SLabelList*	me,
	const char*			barCode, 
	bool_t*				tested,
...
)
{
	...
	if(me->pDBS)
		EXCCHECK( ((SDBSPtr)me->pDBS)->Storage->GetBarCodeHousingStatus(((SDBSPtr)me->pDBS)->Storage, barCode, tested, failed));
...
/*---------------------------------------------------------------------------*/
#undef  __FUNC__
#define __FUNC__	"labels_GetBarCodeTopCoverStatus"
static SElExceptionPtr labels_GetBarCodeTopCoverStatus(
	struct _SLabelList*	me,
	const char*			barCode, 
	bool_t*				tested,
...
)
{
	...
	if(me->pDBS)
		EXCCHECK( ((SDBSPtr)me->pDBS)->Storage->GetBarCodeTopCoverStatus(((SDBSPtr)me->pDBS)->Storage, barCode, tested, failed));
...
)
{
	...
	//EXCCHECK( printer->Reset(printer));
	//Sleep(2000);
