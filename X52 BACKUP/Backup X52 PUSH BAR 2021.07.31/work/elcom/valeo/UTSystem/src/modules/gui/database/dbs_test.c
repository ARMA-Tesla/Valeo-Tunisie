...
#define BKTREE_HEADER0 _("root|Step^w200¦edit|E^w0¦name|Test/Parameter^w250¦desc|Description^w250¦value|Value^w250¦path|Path^w200¦info|I^w22\n")
#define BKTREE_HEADER1 "|1|Steps^i0^r0\n"

#define BKTREE_MENU_ROOT _("New|<Product View>")
#define BKTREE_MENU_TEST _("New Parameter|New Test Before|New Test After|Edit Test|Skip Test|Remove Skip|Delete Test|Move Test|<Parameter View>|<Product View>")
#define BKTREE_MENU_PARAMETER _("New Parameter|Edit Parameter|Delete Parameter|<Parameter View>")
...
typedef enum _EBkTreeMenuTest
{
	...
	BKTREE_MENU_TEST_PRODUDUCTVIEW
} EBkTreeMenuTest;
...
SElExceptionPtr TreeStepsIni(void)
{
	...
	found = sscanf(global->keyNode[EDIT_TEST], "1_%d_%d", &a, &b);
	/* collapse all */
	do
	{
...
SElExceptionPtr BkTreeStepsNodeClick (char *keyNode)
{
	...
	//level = sscanf(keyNode, "1_%d_%d", &a, &b); TEST
	level = sscanf(keyNode, "1_%d", &a);	
...
static SElExceptionPtr MoveTestMode(STestStep test, int32_t editable, const char* keyNode)
{
	SElExceptionPtr	pexception = NULL;
	int32_t			error = 0;
	HRESULT			hr;
...
SElExceptionPtr BkTreeStepMove(int32_t key_id, bool_t *activated)
{
	...
	if(gs_STestMove.active)
	{
		...
		level = sscanf(gs_STestMove.keyNode, "1_%d", &a);
	...
	{	
		...
		switch(menuItem)
		{
			...
			case BKTREE_MENU_TEST_PARAMETERVIEW:
				...
				break;
			case BKTREE_MENU_TEST_PRODUDUCTVIEW:
...
static SElExceptionPtr TreeGetUpperLevelString(const char* keyNode, char* upperlevel)
{
	...
	char			keynode[32];
	char			number[8];
	int32_t			tree[DBS_NUMBER_OF_GROUPS]; 
	int32_t			treeSize = 0;
