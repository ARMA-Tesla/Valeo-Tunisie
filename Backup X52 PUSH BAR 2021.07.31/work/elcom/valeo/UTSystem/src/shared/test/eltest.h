...
int Inhibit_Cylinder_Lock_Up[7]; 
...
typedef struct _STestParam {
	...
	char				barcode_top_cover[256+1];
	char				barcode_housing[256+1];
	char				Barcode[256];
	char				Time[32];
	char				Date[32];
	...
	char				Name_Fault[256];
	char                Production_Serial[5+1];  
	char                ProductType[10+1];  
...
SElExceptionPtr CameraGetdataX52_7(
	SCamerasPtr			pcameras,
	int32_t 			step,
	int32_t				timeout,
	char				*data
);
SElExceptionPtr CameraGetdataX52_9(
	SCamerasPtr			pcameras,
	int32_t 			step,
	int32_t				timeout,
	char				*data
);
SElExceptionPtr CameraGetdataVS11(
	SCamerasPtr			pcameras,
	int32_t 			step,
	int32_t				timeout,
	char				*data
...
SElExceptionPtr CameraCheckJob_X52(
 SCamerasPtr   pcameras
);
SElExceptionPtr CameraSetNubrBouton(
	SCamerasPtr			pcameras,
	int32_t 			type,
	int32_t				timeout
);
SElExceptionPtr LoadJobCamera2 (SCamerasPtr			pcameras,char *job);
...
#define TEST_USE_AXIS			SaxissPtr	paxiss = (SaxissPtr)gs_Modules->Get(gs_Modules, MODULE_AXIS)
...
#define TEST_USE_DAQ2			ViSession	vi_DAQ2 = 1
...
#define TESTER_PUNZONI_DOWN(timeout) \
	EXCCHECK( ptester->Clipping_punzoni(ptester, timeout))

#define TESTER_PUNZONI_UP(timeout) \
	EXCCHECK( ptester->StopClipping_punzoni(ptester, timeout))

#define TESTER_CONTRASTI_UP(timeout) \
	EXCCHECK( ptester->Clipping_contrasti(ptester, timeout))

#define TESTER_CONTRASTI_DOWN(timeout) \
	EXCCHECK( ptester->StopClipping_contrasti(ptester, timeout))

#define TESTER_CHECK_CLIPPING(timeout) \
	EXCCHECK( ptester->CheckClipping(ptester, timeout))

#define TESTER_STOP_CHECK_CLIPPING(timeout) \
	EXCCHECK( ptester->StopCheckClipping(ptester, timeout))

#define TESTER_SELECT_RODAGE_X52(timeout) \
	EXCCHECK( ptester->SelectRodageX52(ptester, timeout))

#define TESTER_SELECT_RODAGE_VS11(timeout) \
	EXCCHECK( ptester->SelectRodageVS11(ptester, timeout))

#define TESTER_RODAGE(timeout,cycle,timeon,timeoff,version) \
	EXCCHECK( ptester->Rodage(ptester, timeout, cycle, timeon, timeoff, version))

#define TESTER_SELECT_STATION_40_VS11(timeout) \
	EXCCHECK( ptester->SelectVS11Station40(ptester, timeout))

#define TESTER_SELECT_STATION_40_X52(timeout) \
	EXCCHECK( ptester->SelectX52Station40(ptester, timeout))

#define TESTER_PLUG_PANEL_STATION_40_VS11(timeout) \
	EXCCHECK( ptester->PlugPanelStation40_VS11(ptester, timeout))

#define TESTER_UNPLUG_PANEL_STATION_40_VS11(timeout) \
	EXCCHECK( ptester->UnplugPanelStation40_VS11(ptester, timeout))

#define TESTER_PLUG_PANEL_STATION_40_X52(timeout) \
	EXCCHECK( ptester->PlugPanelStation40_X52(ptester, timeout))

#define TESTER_UNPLUG_PANEL_STATION_40_X52(timeout) \
	EXCCHECK( ptester->UnplugPanelStation40_X52(ptester, timeout))

#define TESTER_MOVE_X_STATION_40(position, timeout) \
	EXCCHECK( ptester->Move_axis_x_station_40(ptester, position, timeout))

#define TESTER_MOVE_Y_STATION_40(position, timeout) \
	ptester->Move_axis_y_station_40(ptester, position, timeout); \
	EXCCHECK( ptester->Move_axis_y_station_40(ptester, position, timeout))

#define TESTER_MOVE_Z_STATION_40(position, timeout) \
	EXCCHECK( ptester->Move_axis_z_station_40(ptester, position, timeout))

#define TESTER_MOVE_X_STATION_50(position, timeout) \
	EXCCHECK( ptester->Move_axis_x_station_50(ptester, position, timeout))

#define TESTER_MOVE_Y_STATION_50(position, timeout) \
	ptester->Move_axis_y_station_50(ptester, position, timeout); \
	EXCCHECK( ptester->Move_axis_y_station_50(ptester, position, timeout))

#define TESTER_MOVE_Z_STATION_50(position, timeout) \
	EXCCHECK( ptester->Move_axis_z_station_50(ptester, position, timeout))

#define TESTER_SELECT_STATION_50_VS11(timeout) \
	EXCCHECK( ptester->SelectVS11Station50(ptester, timeout))

#define TESTER_SELECT_STATION_50_X52(timeout) \
	EXCCHECK( ptester->SelectX52Station50(ptester, timeout))

#define TESTER_PLUG_PANEL_STATION_50_VS11(timeout) \
	EXCCHECK( ptester->PlugPanelStation50_VS11(ptester, timeout))

#define TESTER_UNPLUG_PANEL_STATION_50_VS11(timeout) \
	EXCCHECK( ptester->UnplugPanelStation50_VS11(ptester, timeout))

#define TESTER_PLUG_PANEL_STATION_50_X52(timeout) \
	EXCCHECK( ptester->PlugPanelStation50_X52(ptester, timeout))

#define TESTER_UNPLUG_PANEL_STATION_50_X52(timeout) \
	EXCCHECK( ptester->UnplugPanelStation50_X52(ptester, timeout))

#define TESTER_SELECT_STATION_60_VS11(timeout) \
	EXCCHECK( ptester->SelectVS11Station60(ptester, timeout))

#define TESTER_SELECT_STATION_60_X52(timeout) \
	EXCCHECK( ptester->SelectX52Station60(ptester, timeout))

#define TESTER_PLUG_PANEL_STATION_60_VS11(timeout) \
	EXCCHECK( ptester->PlugPanelStation60_VS11(ptester, timeout))

#define TESTER_UNPLUG_PANEL_STATION_60_VS11(timeout) \
	EXCCHECK( ptester->UnplugPanelStation60_VS11(ptester, timeout))

#define TESTER_PLUG_PANEL_STATION_60_X52(timeout) \
	EXCCHECK( ptester->PlugPanelStation60_X52(ptester, timeout))

#define TESTER_UNPLUG_PANEL_STATION_60_X52(timeout) \
	EXCCHECK( ptester->UnplugPanelStation60_X52(ptester, timeout))

#define TESTER_ILLUMINATION_STATION_60(status) \
	EXCCHECK( ptester->IlluminationStation60(ptester, status))

#define TEST_CHECKER_RIGHT(output0, output1, output2, output3) \
	EXCCHECK( ptester->TestChecker(ptester, 0, &output0, &output1, &output2, &output3));

#define TEST_CHECKER_LEFT(output0, output1, output2, output3) \
	EXCCHECK( ptester->TestChecker(ptester, 1, &output0, &output1, &output2, &output3));

#define TEST_CHECKER_CENTRAL(output0, output1, output2, output3) \
	EXCCHECK( ptester->TestChecker(ptester, 2, &output0, &output1, &output2, &output3));
...
#define TEST_END_STATION(istation) \
	...
	TEST_DURATION((double)(clock()-_start_test_clock)/(double)CLOCKS_PER_SEC); \
 	if(pexception && ((STestParamPtr)pTID)->fault_continue == FALSE && ((STestParamPtr)pTID)->Test_RnR == FALSE) { TESTER_INIT(istation); \
	SetCtrlIndex( gs_TestPanel, panelStatusStation[istation], 1);} \
 	if(pexception) { Inhibit_Cylinder_Lock_Up [istation] = FALSE;} \
	TEST_FAILED( pexception ); \
    return  pexception
...
#define REQUIRE_JIG if (!error) { \
	SJigPtr pjig = (SJigPtr)gs_Modules->Get(gs_Modules, MODULE_JIG_CLASS);\
	if ( pjig == NULL ) EXCTHROW( TEST_ERROR_NOT_POSIBLE_TEST, _("Jig Module is missing!"))
...
#define REQUIRE_RESISTOR if (!error) { \
	ViSession vi_RESISTOR = 0; \
	SIviBridgePtr pIvi = (SIviBridgePtr)gs_Modules->Get(gs_Modules, MODULE_IVIBRIDGE); \
	if ( pIvi == NULL ) EXCTHROW( TEST_ERROR_NOT_POSIBLE_TEST, _("Ivi Bridge Module is missing!")); \
	EXCCHECK( pIvi->GetSession( pIvi, "RESISTOR", &vi_RESISTOR))
...
#define TESTINIT_END(iStation) \
	...
	TEST_SUB_TEST(_subtest); \
	if(strcmp(_subtest, "90")) EXCCHECKCVI( SetCtrlVal(gs_TestPanel, panelHelpStation[iStation], _test_description));\
	TEST_USED(); \
...
if (((STestParamPtr)pTID)->fault_continue == TRUE) {TEST_FAILED( pexception ); _test_index++;} \
...
#define TEST_SUB_TEST(SubTest) \
	GetSubTest(pTID, ((STestParamPtr)pTID)->step, SubTest)
...
#define TESTSEQUENCE_BARCODE( barcode, size) \
...
#ifdef _CONFIG_FILE_USE_FOR_SWITCHING
	#ifdef CONNECT
	#undef CONNECT
	#define CONNECT(resource) \
		EXCCHECK( SwtchConnect(gs_Modules, CONFIG_FILE, #resource))
	#endif

	#ifdef DISCONNECT
	#undef DISCONNECT
	#define DISCONNECT(resource) \
		EXCCHECK( SwtchDisconnect(gs_Modules, CONFIG_FILE, #resource))
	#endif

	#ifdef IF_CONNECT
	#undef IF_CONNECT
	#define IF_CONNECT(resource) \
		do{ \
			bool_t exist; \
			EXCCHECK( SwtchCheckPath(gs_Modules, CONFIG_FILE, #resource, &exist)); \
			if(exist){
	#endif

	#ifdef IF_NOT_CONNECT
	#undef IF_NOT_CONNECT
	#define IF_NOT_CONNECT(resource) \
		do{ \
			bool_t exist; \
			EXCCHECK( SwtchCheckPath(gs_Modules, CONFIG_FILE, #resource, &exist)); \
			if(!exist){
	...
	#define SWITCH_SEQ(resource) \
		EXCCHECK( SwtchSequence(gs_Modules, CONFIG_FILE, #resource))
#endif
