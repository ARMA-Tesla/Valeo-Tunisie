...
)
{
	...
	EXCCHECKMSXML( MSXML_IXMLDOMDocumentselectSingleNode( ppar->pTicket, NULL, "/testseq/global/parameter[name='JobCamera:Picto']/value", &pnode));
	if(pnode==0)
...
)
{
	...
	double   time;
		...
		EXCCHECK( pcameras->GetCameraByIndex(pcameras, index, &pcamera));	
...
#undef __FUNC__
#define __FUNC__ "LoadJobCamera2"
...
//		}while ((data[9] != 48+step) && (time < timeout));
...
#undef __FUNC__
#define __FUNC__ "CameraGetdataX52_7"
...
)
{
	...
	double   time;
		...
		}while (!strcmp(data,"") && (time < timeout));
		
		if (!strcmp(data,""))
		{
			EXCCHECK( LoadJobCamera2 (pcameras,"001"));
			Sleep(3000);
...
#undef __FUNC__
#define __FUNC__ "CameraGetdataX52_9"
...
#undef __FUNC__
#define __FUNC__ "CameraGetdataVS11"
...
)
{
	...
	double   time;
		...
		if (!strcmp(data,""))
		{
			EXCCHECK( LoadJobCamera2 (pcameras,"002"));
			Sleep(3000);
			EXCTHROW(TEST_ERROR_CAMERA_RESPONSE_TIMEOUT, "Camera Response Timeout!");
		}
Error: 
...
#undef __FUNC__
#define __FUNC__ "CameraCheckJob_X52"
...
)
{
 ...
 int       i=0;
		...
		}while ((job !=1) && (time < 10000));
		
	//	if (job !=1)
	//	{
	//	EXCCHECK( LoadJobCamera2 ("001"));
	//	EXCCHECK( pcamera->Cognex_set_number_boutton(pcamera, 0));
	//	}
Error: 
 EXCRETHROW( pexception); 
...
/*----------------------------------------------------------------------------*/
#undef __FUNC__
#define __FUNC__ "CameraSetNubrBouton"
...
)
{
	...
	EXCCHECK( pcameras->GetCameraByIndex(pcameras, 0, &pcamera));
//	Sleep(100);
	EXCCHECK( pcamera->Cognex_set_number_boutton(pcamera, type));
 //	Sleep(300);
