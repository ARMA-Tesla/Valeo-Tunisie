...
/*---------------------------------------------------------------------------*/
void LogPrinterError( char *Error)   
{
	...
	sprintf(result_dir, "%s\\printer\\%04d.%02d.%02d", project_dir, st.wYear, st.wMonth, st.wDay);
	CheckDirectory(result_dir); 

	
	/* enreg des data */
	memset(log, 0x00, 256);
	sprintf (fileName, "%s/Log_Printer.txt", result_dir);
...
static SElExceptionPtr JigPanelUnlock(int32_t* pError)
{
	...
	bool_t			    barrier, MicroCam;
		...
		//printf("UnlockPanelStation10 Automat.c Line 579  \r \n");
		Sleep(20);
...
static SElExceptionPtr ProcessEventGuiResetProduct(void)
{
	...
	if (StatusStations[1] && StatusStations[2] && StatusStations[3] &&
	    StatusStations[4] && StatusStations[5] && StatusStations[6] && 
		StatusStations[7]) 
	{
	
	PANEL_MSG( PANEL_HELP2, _("Reset Testeur en Cours"));
	 
	/*reset printer */
	EXCCHECK( gs_pTester->Activate_Air_Printer(gs_pTester));
	Sleep(500);
	EXCCHECK( gs_pTester->SetSensor(gs_pTester, "PRINTER_RESET", 1));
	Sleep(100);
	EXCCHECK( gs_pTester->SetSensor(gs_pTester, "PRINTER_RESET", 0));	
	...
	EXCCHECK( gs_paxiss->GetaxisByIndex(gs_paxiss, 5, &paxis));
	...
	EXCCHECK( gs_paxiss->GetaxisByIndex(gs_paxiss, 4, &paxis));
	EXCCHECK( paxis->AlarmReset(paxis, AXIS_Y));
	EXCCHECK( paxis->EnableAxis(paxis, AXIS_Y));
	EXCCHECK( paxis->HomeReturn(paxis, AXIS_Y));
...
static SElExceptionPtr ProcessEventGuiProductSelected(bool_t set_panel, bool_t set_box, bool_t set_master)
{
	...
	//printf("UnlockPanelStation10 Automat.c Line 1138  \r \n");	
	Sleep(20);
...
static SElExceptionPtr ProcessEventJigItemCycleStarted(struct _SSledge* me)
{
	...
	int32_t			erreur;
	int32_t			barrier;
	int32_t			capteur_fermeture_glissiere_imprimante;
...
//#if 0    /* formerly excluded lines */
	   ...
	   if (!strcmp (pthread_data[0]->barcode, ""))
		...
		//printf(" Check Barcode Housing \r \n");
		...
		//printf("JigPanelBarcodeHousing OK \r \n");	
		...
		//Vérification fermeture Glissiere Imprimante 31.07.2021
		gs_pTester->GetSensor(gs_pTester, "SENSOR_SLIDE_PRINTER_CLOSE", &capteur_fermeture_glissiere_imprimante);
		if(!capteur_fermeture_glissiere_imprimante)
		goto Error;
		...
		//printf("UnlockPanelStation10 Automat.c Line 1756 \r\n");
		Sleep(20);
		gs_pTester->UnlockPanelStation10(gs_pTester, 7000);
	...
	//	printf("Insert 1= %d: Insert 2=%d Outside function\n\r",insert1, insert2); 
		...
		{
			/* Unlock palett N°1 */
			//printf("UnlockPanelStation10 Automat.c Line 1769 \r \n");
			Sleep(20);
			gs_pTester->UnlockPanelStation10(gs_pTester, 7000);
...
Error:
	...
	{
		//printf("UnlockPanelStation10 Automat.c Line 1864  \r \n");
		Sleep(20);
		gs_pTester->UnlockPanelStation10(gs_pTester, 7000);
		if(!capteur_fermeture_glissiere_imprimante)
			PANEL_MSG( PANEL_HELP2, _("Glissière Imprimante Ouverte !!!"));
		else 
			PANEL_MSG( PANEL_HELP2, _("Push Start Cycle"));
	}
	StatusStations[0] = TRUE;
...
static SElExceptionPtr ProcessEventJigItemEmpty(void)
{
	...
	//	EXCCHECK( gs_pTester->GetSensor(gs_pTester, "OUTPUT_LASER_RIGHT", &insert2));
		...
		if (insert1 || insert2)
			{
			/* Unlock palett N°1 */
			//printf("UnlockPanelStation10 Automat.c Line 1998 \r \n");
			Sleep(20);
			gs_pTester->UnlockPanelStation10(gs_pTester, 7000);
	 ...
	 if (pexception == NULL && !gs_Automat.disabled)
		...
		gs_Table_Rotation++;
			if (gs_Table_Rotation == NUMBER_OF_STATIONS)
				...
				//printf("UnlockPanelStation10 Automat.c Line 2222 \r \n");
				Sleep(20);
				gs_pTester->UnlockPanelStation10(gs_pTester, 7000);
...
static SElExceptionPtr ProcessEmptyTable(void)
{
	...
	int32_t			insert1=0, insert2=0;
			...
			else
			{
				...
				//printf("UnlockPanelStation10 Automat.c Line 2329 \r \n");
				Sleep(20);
				pexception = gs_pTester->UnlockPanelStation10(gs_pTester, 7000);
	...
	result = WaitForMultipleObjects(EVENT_HANDLE_NB, gs_EventHandle, FALSE, 10000);
		...
		{	
			...
			case WAIT_OBJECT_0 + EVNT_JIG_START_:
				...
				{
					//printf("UnlockPanelStation10 Automat.c Line 2397  \r \n");
					Sleep(20);
					gs_pTester->UnlockPanelStation10(gs_pTester, 7000);
...
static SElExceptionPtr JigPanelBarcodeHousing(struct _SSledge* me, int32_t* pError)
{
	...
	if (!strcmp(me->barcode_housing, "none"))
		{
			printf("!strcmp(me->barcode_housing \r\n");
   ...
   if(gs_Automat.Mode==AUTOMAT_MASTERPART && gs_Automat.master_active && gs_Automat.masterpart)
	...
	{
		//printf("CheckBarCode LabelHousing:Check \r\n");
		...
		{
			bool_t tested;
			//printf("GetBarCodeHousingStatus \r\n");
...
Error: 
	if (pexception)	
	{
		...
		PANEL_MSG( PANEL_HELP2, Error);
		MessagePopup ("Erreur imprimante CAB", Error);
		
		sprintf(log,"automat.c => Printer_Ready_Reset :  StatusOnline = %d , Interpreter = %d \r \n",StatusOnline,Interpreter);
		LogPrinterError(log);
	}
	EXCRETHROW( pexception); 
}
...
Error: 
	...
	if (pexception)	
	{
		...
		sprintf(log,"automat.c => JigPanelLabel :  StatusOnline = %d , Interpreter = %d \r \n",StatusOnline,Interpreter);
		LogPrinterError(log);
	}
	if (pexception)	
	{	
...
Error:
	...
	if (pexception)	
	{
		...
		sprintf(log,"automat.c => JigPanelLabelFail :  StatusOnline = %d , Interpreter = %d \r \n",StatusOnline,Interpreter);
		LogPrinterError(log);
	}	
...
/*---------------------------------------------------------------------------*/
#undef __FUNC__
#define __FUNC__ "GetLastShift"
static SElExceptionPtr GetLastShift (bool_t* IsOpen, EIdShift* shift_id)
{
	SElExceptionPtr	pexception = NULL;
	int32_t			error = 0;
...
static SElExceptionPtr EventFunctionPluginCallback( const char* eventName, void* pData)
{
	...
	else if(0==strcmp(eventName, "EVNT_GUI_SHIFT"))
	{
		...
		if(gs_Automat.master_active) // /* master part active in XML automat.XML */
		{
			if (gs_Last_shift_id != gs_Automat.shift_id) //controle si selection meme shift pas necessaire de demander le mode piece 
			{
				/* set mode passage piece type avant production */
				gs_Automat.Mode = AUTOMAT_MASTERPART;
				
				/* new Last shift ID application is open refresh from here gs_Last_shift_id */
				gs_Last_shift_id = gs_Automat.shift_id; //
			}
			else 
			{
				/* set mode production */
		...
		else /* master part not active in XML */
		{
				/* set mode production */
				gs_Automat.Mode = AUTOMAT_PRODUCTION;	
		}
		
		if( gs_Automat.Mode==AUTOMAT_PRODUCTION) /* mode prod (mster part xml = FALSE et shift selected */
		{
			PANEL_MSG( PANEL_HELP1, _("Production")); 
		...
		else if( gs_Automat.Mode==AUTOMAT_MASTERPART) /* mode prod (mster part xml = TRUE et shift selected */
		{

			if (gs_Automat.masterpart == FALSE) //controle si le mode piece est en cours de travail sinon declenchement mode piece type
				{				
					gs_Automat.masterpart = TRUE;//pour définir le mode de lecture et vérifications des codes à barre Housing + TOP COVER et inhibiter le vidage et le changement de ref en mode pîéce type 
	...
	{
	if (gs_Automat.masterpart == FALSE)
		{
		...
		//	EXCCHECK( gs_pTester->GetSensor(gs_pTester, "OUTPUT_LASER_RIGHT", &insert2));
			...
			{
				//printf("UnlockPanelStation10 Automat.c Line 3487 \r \n");
				Sleep(20);
				pexception = gs_pTester->UnlockPanelStation10(gs_pTester, 7000);
	...
	if (gs_Automat.masterpart == FALSE)
		...
		//	EXCCHECK( gs_pTester->GetSensor(gs_pTester, "OUTPUT_LASER_RIGHT", &insert2));
			...
			{
				//printf("UnlockPanelStation10 Automat.c Line 3529 \r \n ");
				pexception = gs_pTester->UnlockPanelStation10(gs_pTester, 7000);
				Sleep(20);
...
PLUGIN_API SElExceptionPtr PLUGIN_RUN_NAME_CODE( SPluginArgsPtr a_pPluginArgs, const char* uuid)
{
	...
	/* read last shift */
	EXCCHECK( gs_pDbs->Storage->IsShiftOpen(gs_pDbs->Storage, &IsOpen, &shift_id));
	EXCCHECK(GetLastShift (&IsOpen, gs_Last_shift_id));
	gs_Last_shift_id =  shift_id;
	...
	if (Finished_master_parts == number_of_master_parts) //the tour is finished
		{
				...
				{
					int ok  = ConfirmPopup ("MODE PIECE TYPE", "Voulez vous passer en production ('les piéces types sont NOK')");
					
					if (ok == 1) /* demande de passer en prod si master parts NOK */
						{
							/* reload production testsequence */
							gs_Automat.Mode = AUTOMAT_PRODUCTION;
					...
					else /* continuer en mode piéce type et retester les masters parts NOK */
						{
							/* Loop to remove last master panel */
							PANEL_MSG( PANEL_HELP2, _("Remove Panel"));
