...
static SElExceptionPtr storage_GetBarCodeHousingStatus(struct _SDBSStorage* me, const char* barCode, bool_t* tested, bool_t* failed);
static SElExceptionPtr storage_GetBarCodeTopCoverStatus(struct _SDBSStorage* me, const char* barCode, bool_t* tested, bool_t* failed);
...
static SElExceptionPtr ConvertTimestampToANSI2(char* timestamp, time_t* time);
...
SElExceptionPtr dbsstorage_new(SDBSStoragePtr* pDBSStoragePtr, void* pDBS)
{
	...
	me->GetBarCodeHousingStatus = storage_GetBarCodeHousingStatus;
   	me->GetBarCodeTopCoverStatus = storage_GetBarCodeTopCoverStatus;
...
static SElExceptionPtr storage_StartShift(struct _SDBSStorage* me, const EIdShift shift_id)
{
	...
	sprintf(sql, "INSERT INTO valeo.status "
...
static SElExceptionPtr storage_StopShift(struct _SDBSStorage* me)
{
	...
	sprintf(sql, "UPDATE valeo.status "
...
)
{
	...
	if(is_xml_success)
	{
		...
		EXCCHECKMSXML( MSXML_IXMLDOMNodeselectSingleNode (pnode, NULL, "barcode_top_cover", &pparameter)); 
		if(pparameter)
		{
			EXCCHECKMSXML( MSXML_IXMLDOMNodeGettext( pparameter, NULL, &ptext));
			
			sprintf(barcode_top_cover, ptext); 
		...
		EXCCHECKMSXML( MSXML_IXMLDOMNodeselectSingleNode (pnode, NULL, "barcode_housing", &pparameter)); 
		if(pparameter)
		{
			EXCCHECKMSXML( MSXML_IXMLDOMNodeGettext( pparameter, NULL, &ptext));
			
			sprintf(barcode_housing, ptext); 
		...
		if( strlen(date_time)>0
			...
			&& strlen(failed)>0)
		{
			sprintf(sql, "INSERT INTO valeo.product_status "
			   ...
			   && mode_id==ID_MODE_PRODUCTION)
			{
				...
				EXCCHECK( elstring_new("INSERT INTO valeo.test_status VALUES ", &pelstring)); 
...
)
{
	...
	if (t1==NULL && t2==NULL)
	{
		sprintf(sql, "SELECT COUNT(ps.product_id) " 
					 "FROM valeo.product_status ps "
					 "WHERE ps.tester_id=%d AND ps.failed=%d AND mode_id=%d",
					 PDBS_TESTER_ID,
					 (failed)? 1:0,
					 mode_id);
	//	printf("case t1==NULL && t2==NULL sql=%s\r\n",sql);
	}
	else
	{
		char datetime1[DBS_RECORD_LENGHT_TIME+1] = "";
...
static SElExceptionPtr storage_GetLastTwoProducts(struct _SDBSStorage* me, time_t start, time_t* t1, time_t* t2)
{
	...
	if(error!=ELODBC_SQL_WARN_NO_DATA_FOUND)
	{
		EXCCHECK( ConvertTimestampToANSI2( (char*)podbc->GetFieldValuePtr(podbc, 1), t1));
		if(podbc->MoveNext(podbc)==0)
		{	
		//	printf("MoveNext\r\n");
			EXCCHECK( ConvertTimestampToANSI2( (char*)podbc->GetFieldValuePtr(podbc, 1), t2));  
		}
		else
			*t2 = *t1;
...
static SElExceptionPtr storage_GetProductCounter(struct _SDBSStorage* me, int32_t product_id, uint32_t* pCount)
{
	...
	/* lock database valeo.product_counter */
	PDBS_LOCK(LOCK_STR);	
	...
	if(datetime_last[0]!='\0')
	{
		...
		{
			sprintf(sql, "UPDATE valeo.product_counter "
...
/*---------------------------------------------------------------------------*/
#undef __FUNC__
#define __FUNC__ "storage_GetBarCodeHousingStatus"
static SElExceptionPtr storage_GetBarCodeHousingStatus(struct _SDBSStorage* me, const char* barCode, bool_t* tested, bool_t* failed)
{
	SElExceptionPtr	pexception = NULL;   
	int32_t			error = 0;
	...
	if(barCode && *barCode && tested)
	{
		...
		sprintf(sql, "select ps.failed "
					 ...
					 "where ps.barcode_housing='%s' AND ps.mode_id=%d "
...
/*---------------------------------------------------------------------------*/
#undef __FUNC__
#define __FUNC__ "storage_GetBarCodeTopCoverStatus"
static SElExceptionPtr storage_GetBarCodeTopCoverStatus(struct _SDBSStorage* me, const char* barCode, bool_t* tested, bool_t* failed)
{
	SElExceptionPtr	pexception = NULL;   
	int32_t			error = 0;
	...
	if(barCode && *barCode && tested)
	{
		...
		sprintf(sql, "select ps.failed "
					 "from product_status ps "
					 "where ps.barcode_top_cover='%s' AND ps.mode_id=%d "
					 "order by ps.time desc ",
					 barCode,
					 ID_MODE_PRODUCTION);
...
/*---------------------------------------------------------------------------*/
#undef __FUNC__
#define __FUNC__ "ConvertTimestampToANSI2"
static SElExceptionPtr ConvertTimestampToANSI2(char* timestamp, time_t* pTime)
{
	SElExceptionPtr	pexception = NULL;   
	struct tm   stm;
	int			month, day, year;
