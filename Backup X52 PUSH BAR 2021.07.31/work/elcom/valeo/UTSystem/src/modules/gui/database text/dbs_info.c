...
void CVICALLBACK PostDeferredDisplayInfo (void *callbackData)
{
	...
	EXCCHECKCVI( SetPanelAttribute (gs_panel[INFO], ATTR_TOP, top));
	EXCCHECKCVI( SetPanelAttribute (gs_panel[INFO], ATTR_LEFT, left)); 
	...
	EXCCHECKCVI( DisplayPanel (gs_panel[INFO]));
...
void CVICALLBACK PostDeferredHideInfo (void *callbackData)
{
	...
	EXCCHECKCVI( HidePanel (gs_panel[INFO])); 
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
		...
		case EVENT_COMMIT:
			...
			EXCCHECKCVI( GetCtrlVal(panel, INFO_TO_FILE, &value));
...
static SElExceptionPtr CreateTestLimitIniFile(bool_t to_file, time_t time)
{
	...
	if(valid)
	{
		...
		EXCCHECK( PEXPORT->GetTestSeq(PEXPORT, product_id, time, &etest, &etestSize)); 
