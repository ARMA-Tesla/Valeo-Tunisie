...
)
{
	...
	if(pTestSeq->_SetPanelStationStatus)
...
)
{
	...
	pTestParam->Retest_Check_MAN_DATA_2 = 0;
    ...
    if ( ptest )
	{
		for(i=0; i<pTestSeq->_TestCount; i++, ptest++)
	    {
			...
			else if (pTestParam->Skip_All_Test == TRUE && pTestParam->Check_DTC == TRUE)
			{
				sscanf((*ptest)->test.name, "%d.%d", &Test, &SubTest); // to change  StatusStations from 0 to 1
	
			 	if (!strcmp ((*ptest)->test.name, "30.38") || //check DTC in station 30
					!strcmp ((*ptest)->test.name, "30.02") ||
					!strcmp ((*ptest)->test.name, "30.03") ||
					!strcmp ((*ptest)->test.name, "30.04")) 
			 	{
				...
				else if (SubTest == 90) /* if all test skipped Run Only wait Table */ 
				{
				StatusStations[(Test/10)-1] = 1; 
				pexception = ( (*ptest)->Run( *ptest, pTestParam, &handle, NULL ));
				if (fault_ignore)
			...
			else if (pexception == NULL && pTestParam->Skip_All_Test == FALSE) /* if panel Pass Run next step */
			{
				sscanf((*ptest)->test.name, "%d.%d", &Test, &SubTest); // to change  StatusStations from 0 to 1
				if (SubTest == 90)
				{
				...
				else
				{
					...
					else 
					{
						if ((*ptest)->_IsUsed)
							...
							if (fault_ignore &&   /* don't run Rodage for master parts & expertise*/
				  			   (!strcmp((*ptest)->test.name, "20.00") ||
								!strcmp((*ptest)->test.name, "20.01") ||
								!strcmp((*ptest)->test.name, "20.20") ||
								!strcmp((*ptest)->test.name, "50.00") ||
								!strcmp((*ptest)->test.name, "50.01") ||
								!strcmp((*ptest)->test.name, "50.10")));
							
							
							else if (!fault_ignore &&  /* retest en prod pas en mode GO */ 
									 (!strcmp((*ptest)->test.name, "30.40") ||  
									  !strcmp((*ptest)->test.name, "40.40")))
							{
								retest = 0;
								pTestParam->fault_retest = TRUE;
...
#if 0    /* formerly excluded lines */
				else if (pexception == NULL ) /* if panel Pass Run next step */
				{
#endif   /* formerly excluded lines */
				/* test RnR */
