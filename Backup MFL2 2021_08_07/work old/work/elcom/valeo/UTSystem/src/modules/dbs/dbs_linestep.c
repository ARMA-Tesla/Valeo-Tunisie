...
static SElExceptionPtr step_LineStepSave(struct _SDBSLineStepList* me, int32_t branch_id)
{
	...
	if(me->LineStep)
	{
		...
		/* get last test_id */
		EXCCHECK( podbc->ExecSQL( podbc, "SELECT MAX(ls.fnc_id) FROM valeo.line_steps ls"));
		CHECKERR( podbc->Fetch( podbc, TRUE));
		...
		{	
			...
			sprintf(sql, "REPLACE INTO valeo.line_steps "
