...
static SElExceptionPtr camera_Cognex_set_live(SCameraPtr me, int phase);
...
)
{
	...
	pnewcamera->Cognex_set_live		= camera_Cognex_set_live;
...
)
{
	...
	if (me->specLib == 0)
	{
		...
		me->_Cognex_set_liveSpec = (SElExceptionPtr (*)(SCameraPtr,int))
								GetProcAddress(me->specLib, "Cognex_set_live");
...
#undef __FUNC__
#define __FUNC__ "{Camera}::Cognex_set_live"
static SElExceptionPtr camera_Cognex_set_live(SCameraPtr me, int live)
{
	SElExceptionPtr	pexception = NULL;


	if (me->_Cognex_set_liveSpec)
		EXCCHECK( me->_Cognex_set_liveSpec(me,live));
