...
static SElExceptionPtr param_LineParamSave(struct _SDBSLineParamList* me, int32_t branch_id)
{
	...
	if(me->LineParam)
	{
		...
		/* get last test_id */
		EXCCHECK( podbc->ExecSQL( podbc, "SELECT MAX(lp.parameter_id) FROM valeo.line_parameters lp"));
		CHECKERR( podbc->Fetch( podbc, TRUE));
		...
		{	
			...
			sprintf(sql, "REPLACE INTO valeo.line_parameters "
			...
			for(; i<me->LineParamSize; i++)
			{
				...
				sprintf(sql, ",(%d,%d,\"%s\",_latin2\"%s\",%d,%d,\"%s\",%d)", 
							 me->LineParam[i].product_id,
							 me->LineParam[i].fnc_nb,
							 me->LineParam[i].name,			
							 me->LineParam[i].value,
