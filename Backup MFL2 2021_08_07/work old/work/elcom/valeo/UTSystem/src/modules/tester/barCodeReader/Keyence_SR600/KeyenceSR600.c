...
#define BARCODE_TIMEOUT		3000
 
#define TIMEOUT_BARCODE_INIT() 	clock_t	_start_clock = clock();

#define TIMEOUT_BARCODE_CHECK(timeout) \
	if ( ((clock() - _start_clock) / (double)CLOCKS_PER_SEC) > (timeout/1000.0) ) { EXCTHROW( BCR_ERROR_WRITE_TIMEOUT, "Barcode Timeouted"); } else
