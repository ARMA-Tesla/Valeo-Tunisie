...
static SElExceptionPtr	Measure_Mean( int ichannel, ViReal64 *moyenne)
{
	...
	int				i;
   	ViReal64		rvalue;
	ViReal64		somme = 0;
	double         Input_data[SAMPLES];
	double         Output_data[SAMPLES] = {0};

	
	for ( i = 0; i < SAMPLES; i++)
	{
		Input_data[i] = data_Analog[ichannel*20];
		Sleep (1);
	}
	 
	
	LineaTable1D (Input_data, pScaleInfo[ichannel], SAMPLES, Output_data);
	
	if (pScaleInfo[ichannel])	 
	{
		for ( i = 0; i < SAMPLES; i++)
			somme = somme + Output_data[i];
	}
	else
	{
		for ( i = 0; i < SAMPLES; i++)
			somme = somme + Input_data[i];
	}
