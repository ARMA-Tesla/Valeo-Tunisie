...
)
{
	...
	if (bcr_list)
	{
		...
		EXCCHECK( me->jig_SetSensor(me, "DisableLaser1", 1));
		EXCCHECK( me->jig_SetSensor(me, "DisableLaser2", 1));
	}
...
SElExceptionPtr Destroy(struct _SJig* me)
{
	...
	EXCCHECK( me->jig_SetSensor(me, "DisableLaser1", 0));
	EXCCHECK( me->jig_SetSensor(me, "DisableLaser2", 0));
