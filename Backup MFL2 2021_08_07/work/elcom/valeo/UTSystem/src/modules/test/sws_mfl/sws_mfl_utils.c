...
SElExceptionPtr CheckAxis( SaxisPtr paxis, int axis, STesterPtr ptester, double position, double timeout)
{
	SElExceptionPtr pexception = NULL;
	double Initial_Position;
	double Currentposition;
...
SElExceptionPtr MoveToPush(SaxisPtr	paxisZ , SaxisPtr	paxisXY, STesterPtr ptester, int check, 
						   double position_axis_Z, double position_axis_Y, double position_axis_X)
{
	...
	if (check == 1)
	{
		EXCCHECK(CheckAxis (paxisZ, AXIS_Z, ptester, position_axis_Z, 5000));
		EXCCHECK(CheckAxis (paxisXY, AXIS_Y, ptester, position_axis_Y, 5000));
		EXCCHECK(CheckAxis (paxisXY, AXIS_X, ptester, position_axis_X, 5000));
	}
	else
	{
	EXCCHECK(MoveAxis (paxisZ, AXIS_Z, position_axis_Z));
	EXCCHECK(MoveAxis (paxisXY, AXIS_Y, position_axis_Y));
	EXCCHECK(MoveAxis (paxisXY, AXIS_X, position_axis_X));
	}
