...
static SElExceptionPtr labels_printLabel2(
	SLabelListPtr	me,
	SPrinterPtr		printer,
	const char*		labelId,
	int32_t			productId,
...
)
{
	...
	plabels->PrintLabel2		= labels_printLabel2;
...
)
{
	...
	if (p2label_def==NULL 
		|| p2label_def->text==NULL 
		|| 0==strlen(p2label_def->text))
	{
		EXCTHROW(-1, "Label Definition Not Found");
	}
	...
	if(parameters!=NULL)
	{
	 ...
	 print_label = ParseLabel(me, p2label_def->text, p2label, document);
	
	 StreamFile = fopen ("C:\\Keyence\\temp.txt", "w");
	
	 fwrite (print_label, sizeof(char), strlen(print_label), StreamFile);
	
	 fclose (StreamFile);

	 Sleep(500);
	
	 /* Open File Laser Stream */
	 FileTemp = OpenFile ("C:\\Keyence\\temp.txt", VAL_READ_WRITE, VAL_OPEN_AS_IS, VAL_ASCII);
	
	 FileHandle = OpenFile ("C:\\Keyence\\MFL_Laser_data.txt", VAL_READ_WRITE, VAL_TRUNCATE, VAL_ASCII);
	 
	 /* Read Line 1 */
	 ReadLine (FileTemp, Line1, -1);
	
	 WriteLine (FileHandle, Line1, -1);
	 
	 Sleep(20);
	 
	 /* Read Line 2 */
	 size_Line2 = ReadLine (FileTemp, Line2, -1);
	 
	 for (i=size_Line2; i<60; i++)
	 	Line2[i] = '0';
	
	 Line2[i] = 0X00;
	 
	 WriteLine (FileHandle, Line2, -1);
	 
	 Sleep(20);

 	 /* Read Line 3 */
	 size_Line3 = ReadLine (FileTemp, Line3, -1);
	 
	 /*for (i=size_Line3; i<60; i++)
	 	Line3[i] = '0';*/
	 if(size_Line3>0)
	 {
		 strcat( Line3,G1) ;
		 size_Line3=strlen(Line3);
		 for (i=size_Line3; i<60; i++)
			 Line3[i] = '0';
	 }
	 ...
	 WriteLine (FileHandle, Line3, -1);
	 
	 Sleep(20);

	 /* Read Line 4 */
	 ReadLine (FileTemp, Line4, -1);
	
	 WriteLine (FileHandle, Line4, -1);
	 
	 Sleep(20);
	 /* Read Line 5 */
	 
	 ReadLine (FileTemp, Line5, -1);
	 WriteLine (FileHandle, Line5, -1);
	 
	 Sleep(20);
	 /* Read Line 6 */
	 ReadLine (FileTemp, Line6, -1);
	
	 WriteLine (FileHandle, Line6, -1);
	 
	 Sleep(20);
	 /* Read Line 7 */
	 ReadLine (FileTemp, Line7, -1);
	
	 WriteLine (FileHandle, Line7, -1);
	 
	 Sleep(20);
	 /* Read Line 8 */
	 ReadLine (FileTemp, Line8, -1);
	
	 WriteLine (FileHandle, Line8, -1);
	 
	 Sleep(20);
	 

	 /* close File Laser Stream*/
	 CloseFile (FileTemp);
...
/*---------------------------------------------------------------------------*/
#undef  __FUNC__
#define __FUNC__	"labels_printLabel3"
static SElExceptionPtr labels_printLabel3(
	SLabelListPtr	me,
	SPrinterPtr		printer,
	const char*		labelId,
	int32_t			productId,
...
/*---------------------------------------------------------------------------*/
#undef  __FUNC__
#define __FUNC__	"labels_printLabel2"
static SElExceptionPtr labels_printLabel2(
	SLabelListPtr	me,
	SPrinterPtr		printer,
	const char*		labelId,
...
)
{
	...
	else  //from database!
	{
		...
		else
		{
		   ...
		   if (!strcmp(variableName, "Variant Code"))
 			   CopyBytes(new_value,0, p2param->value, 1, 1);
		   
		   else if (!strcmp(variableName, "BMW_SW_Revision"))
 			   CopyBytes(new_value,0, p2param->value, 0, 2);
		   
		   else if (!strcmp(variableName, "BMW_HW_Revision"))
		   {
 			   CopyBytes(new_value,0, p2param->value, 0, 2);
			   if (productParams != NULL && productParams->parameters != NULL && productParams->paramSize != 0)
			       p3param = FindParameterByName(productParams->parameters, productParams->paramSize, "laser_indice");
			   if (p3param == NULL)
					{
						sprintf (G1,"\0");
					}
					else
					{
			   			...
			   			sprintf (G1,p3param->value);
							 
			   		}
				//printf ("G1");	
			
			   /*if(!strcmp(p2param->value,"0701"))
			   {
				   //
				   sprintf (G1,"G1");
			   }
		   ...
		   else if (!strcmp(variableName, "ProductPartNumber"))
		   {
			   if (strlen (p2param->value) == 8) // exemple 42055700
			   {
				size = strlen(p2param->value) + 2;
				new_value = malloc(size * sizeof(char));
			
				memset(new_value, 0x00, size);
				   
				new_value[0] = '0'; // add '0' instead of E
 			   	CopyBytes(new_value,1, p2param->value, 0, 8);
			   }
			   else if (strlen (p2param->value) == 9) // exemple E12509400
			   {
 			   	CopyBytes(new_value,0, p2param->value, 0, 9);
			   }
		   }
