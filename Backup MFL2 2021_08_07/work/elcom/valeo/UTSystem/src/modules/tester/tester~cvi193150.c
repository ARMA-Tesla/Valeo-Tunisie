...
static SElExceptionPtr IsTableRotated(struct _STester* me, int32_t timeout);
 ...
 SElExceptionPtr tester_new(STesterPtr*	pTester)
{
	...
	*pTester = calloc( 1, sizeof(STester));
	...
	(*pTester)->IsTableRotated = IsTableRotated;
...
/*----------------------------------------------------------------------------*/
#undef __FUNC__
#define __FUNC__ "Thread_TableRotation"
static DWORD WINAPI Thread_TesterTableRotate(LPVOID param)
{
	...
	bool_t			is_z_home_right_station_30,
			...
			//EXCCHECK( me->GetSensor(me, "HEND_Z_RIGHT_STATION_30", &is_z_home_right_station_30));
			//EXCCHECK( me->GetSensor(me, "HEND_Z_LEFT_STATION_30", &is_z_home_left_station_30));	
			...
			if(is_panel_unplugged_station_30 && !is_panel_plugged_station_30 && 
			   position_axis_Z_right_station_30 < 0.1 && position_axis_Z_left_station_30 < 0.1) 
			{
			...
			//EXCCHECK( me->GetSensor(me, "HEND_Z_RIGHT_STATION_40", &is_z_home_right_station_40));
			//EXCCHECK( me->GetSensor(me, "HEND_Z_LEFT_STATION_40", &is_z_home_left_station_40));
			...
			if(is_panel_unplugged_station_40 && !is_panel_plugged_station_40 && 
			   position_axis_Z_right_station_40 < 0.1 && position_axis_Z_left_station_40 < 0.1)
			{
...
static SElExceptionPtr TableRotate(struct _STester* me, int32_t timeout, bool_t wait_end)
{
	...
	if(me->hThreadTableRotate == NULL)
	{
		EXCCHECK( ThreadStart(Thread_TesterTableRotate, pthread_data, &me->hThreadTableRotate, NULL));

		if(wait_end)
		{
		pexception =  me->IsTableRotated(me, timeout + 1000);
		if(me->hThreadTableRotate)
		{
			CloseHandle(me->hThreadTableRotate);
			me->hThreadTableRotate = NULL;
		}
		me->SetSensor(me, "FORCAGE_ROTATION", 0);
		me->SetSensor(me, "START_ROTATION", 0);
		EXCRETHROW(pexception);
...
/*---------------------------------------------------------------------------*/
#undef __FUNC__
#define __FUNC__	"Tester::IsTableRotated"
static SElExceptionPtr IsTableRotated(struct _STester* me, int32_t timeout)
{
	SElExceptionPtr	pexception = NULL;
	int32_t			status;
		
	pexception = ThreadStatus(me->hThreadTableRotate, timeout, &status);
	if(pexception)
	{
...
//	me->MoveBarcodeRear(me, 2000);
	
	/* check if laser started */
#if 0    /* formerly excluded lines */
	_start_clock =  clock();
	do{
		me->SetSensor(me, "START_LASER", 0);
		Sleep (500);
		me->SetSensor(me, "START_LASER", 1);
		Sleep (500);
		me->GetSensor(me, "LASER_WORK", &laser_work);
		time = (double)(clock()-_start_clock);
	}while (laser_work == 0 && time < timeout);
	 
	me->SetSensor(me, "START_LASER", 0);

	if (time >= timeout)
	{
		EXCTHROW( TESTER_ERROR_TIMEOUT, "laser not started");
	}
	
	me->SetSensor(me, "ASPIRATOR", 1);
