		...
		case EVENT_COMMIT:
			...
			switch(control)
			{
				case HIST_PAGE:
					break;
				case HIST_NEXT:
					EXCCHECKCVI( GetNumListItems (panel, HIST_PAGE, &items));
					EXCCHECKCVI( GetCtrlVal (panel, HIST_PAGE, &value)); 
					if(value==items-1)
						EXCCHECKCVI( SetCtrlIndex (panel, HIST_PAGE, 0));
					else
						EXCCHECKCVI( SetCtrlIndex (panel, HIST_PAGE, value+1));
					break;
				case HIST_PREW:
					EXCCHECKCVI( GetNumListItems (panel, HIST_PAGE, &items));
					EXCCHECKCVI( GetCtrlVal (panel, HIST_PAGE, &value)); 
					if(value==0)
						EXCCHECKCVI( SetCtrlIndex (panel, HIST_PAGE, items-1));
					else
						EXCCHECKCVI( SetCtrlIndex (panel, HIST_PAGE, value-1));
					break;
			}
...
void CVICALLBACK PostDeferredDisplayHistory (void *callbackData)
{
	...
	EXCCHECKCVI( SetPanelAttribute (gs_panel[HIST], ATTR_TOP, top));
	EXCCHECKCVI( SetPanelAttribute (gs_panel[HIST], ATTR_LEFT, left)); 
	
	EXCCHECKCVI( DisplayPanel (gs_panel[HIST]));  
...
void CVICALLBACK PostDeferredHideHistory (void *callbackData)
{
	...
	EXCCHECKCVI( HidePanel (gs_panel[HIST]));
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
		...
		case EVENT_COMMIT:
			...
			EXCCHECKCVI( GetCtrlVal(gs_panel[HIST], HIST_SWITCH_PROD, &value));  
			if(!value) 
				gs_his_filter |= HIS_SHOW_PRODUCT;
			
			GetCtrlVal(gs_panel[HIST], HIST_SWITCH_PRODPARAM, &value); 
			if(!value)
				gs_his_filter |= HIS_SHOW_PRODUCT_PARAMETER;
			
			GetCtrlVal(gs_panel[HIST], HIST_SWITCH_PRODMASTER, &value); 
			if(!value)
				gs_his_filter |= HIS_SHOW_PRODUCT_MASTER;
			
			GetCtrlVal(gs_panel[HIST], HIST_SWITCH_TEST, &value);  
			if(!value)
				gs_his_filter |= HIS_SHOW_TEST;
			
			GetCtrlVal(gs_panel[HIST], HIST_SWITCH_TESTPARAM, &value);   
			if(!value)
				gs_his_filter |= HIS_SHOW_TEST_PARAMETER;
			
			GetCtrlVal(gs_panel[HIST], HIST_SWITCH_USER, &value);   
			if(!value)
				gs_his_filter |= HIS_SHOW_USER;
		
			GetCtrlVal(gs_panel[HIST], HIST_SWITCH_USERRIGHTS, &value);   
			if(!value)
...
static SElExceptionPtr	FilePage(int32_t phistorySize)
{
	...
	EXCCHECKCVI( ClearListCtrl (gs_panel[HIST], HIST_PAGE)); 
	
	for(i=0; i<temp.quot+1; i++)
	{
		sprintf(name, "%02d/%02d", i+1, temp.quot+1);
		EXCCHECKCVI( InsertListItem (gs_panel[HIST], HIST_PAGE, -1, name, i));
	}
