...
)
{
	...
	if(is_xml_success)
	{
		...
		EXCCHECKMSXML( MSXML_IXMLDOMNodeselectSingleNode (pnode, NULL, "barcode_Right", &pparameter)); 
		if(pparameter)
		{
			EXCCHECKMSXML( MSXML_IXMLDOMNodeGettext( pparameter, NULL, &ptext));
			
			sprintf(barcode_Right, ptext); 
		...
		EXCCHECKMSXML( MSXML_IXMLDOMNodeselectSingleNode (pnode, NULL, "barcode_Left", &pparameter)); 
		if(pparameter)
		{
			EXCCHECKMSXML( MSXML_IXMLDOMNodeGettext( pparameter, NULL, &ptext));
			
			sprintf(barcode_Left, ptext); 
		...
		if( strlen(date_time)>0
			&& strlen(product_id)>0
			&& strlen(barcode)>0
			&& strlen(barcode_Left)>0
			&& strlen(barcode_Right)>0
			&& strlen(duration)>0
			&& strlen(failed)>0)
		{
...
static SElExceptionPtr storage_GetProductCount(		//OUSSAMA
	struct _SDBSStorage* me, 
	time_t* t1, 
	time_t* t2, 
	uint16_t failed,
...
//	printf("Product counter not created\n\r");
