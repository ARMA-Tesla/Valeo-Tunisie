...
static SElExceptionPtr axis_CurrentAnalog(SaxisPtr me, int32_t axis, double* analog, double Coefficient, double Offsets);
...
)
{
	...
	pnew_axis->CurrentAnalog			= axis_CurrentAnalog;
...
)
{
	...
	me->_CurrentAnalogSpec = (SElExceptionPtr (*)(SaxisPtr, int32_t, double*, double, double))
								GetProcAddress(me->specLib, "CurrentAnalog");
	...
	else if (!stricmp(xml_type, "CAN"))
	{
		...
		{
			EXCCHECKCOM( MSXML_IXMLDOMNodeGettext(xml_node, NULL, &xml_value));
			sprintf(me->interfaceCAN , xml_value);
		}
		...
		{
			EXCCHECKCOM( MSXML_IXMLDOMNodeGettext(xml_node, NULL, &xml_value));
			me->ID_SMAC = atol(xml_value);
			CA_FREE_MEMORY(xml_value);
		}
		
		if (me->_InitializeSpec)
			EXCCHECK( me->_InitializeSpec(me, ID_NODE));
	}
...
#undef __FUNC__
#define __FUNC__ "{axis}::CurrentAnalog"
static SElExceptionPtr axis_CurrentAnalog(SaxisPtr me, int32_t axis, double* analog, double Coefficient, double Offsets)
{
	SElExceptionPtr	pexception = NULL;
	...
	if (me->_CurrentPositionSpec)
		EXCCHECK( me->_CurrentAnalogSpec(me, axis, analog, Coefficient, Offsets));
