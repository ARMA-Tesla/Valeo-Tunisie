...
static int ReadResponse (int read)
{
    ...
    // Read response from telnet server 
        bytesRead = 0;
        error = InetTelnetRead (gTelnetHandle, response, read - 1, &bytesRead, 8000);
...
SElExceptionPtr Cognex_login(SCameraPtr me)
{
	...
	int Status;	
		...
		if (gTelnetHandle >= 0)
     	{
		// login
		InetTelnetWrite (gTelnetHandle, COGNEX_LOGIN, "", -1, 0, 1000);	
		ReadResponse (sizeof("Password:"));
		
		Delay (0.1);
			
		// pass
		InetTelnetWrite (gTelnetHandle, COGNEX_PASS, "", -1, 0, 1000);	
...
SElExceptionPtr Cognex_Read(SCameraPtr me, char *data)
{
	...
	}while (cameraDataReady == 0 && time < 2000);
	
	if (cameraDataReady == 1)
	{
		cameraDataReady = 0;
		memset(data, 0x00, 128);
		CopyBytes (data, 0, cameraData, 0, 18);
	}
	else  //try to read with Telnet
	{
    InetTelnetRead (gTelnetHandle, response, 18 - 1,&bytesRead, 2000);
    ProcessResponse(response);	
	CopyBytes (data, 0, response, 0, 18);
	//printf ("try to read with Telnet : data = %s", data);	
	}
