...
)
{
	...
	sprintf(sql, "SELECT COUNT(pp.product_id) "
				 "FROM valeo.product_parameters pp "
...
)
{
	...
	if(!time) 
	{
		...
		EXCCHECK( export_fncGetTestSeq(me, product_id, testseq, testseqSize) );
	}
	else
...
/* <product> */
	...
	EXCCHECK(elstring->Append(elstring, "		<pallet></pallet>\n") ); 
	EXCCHECK(elstring->Append(elstring, "		<EMS_PN_PCBA></EMS_PN_PCBA>\n") ); 
	EXCCHECK(elstring->Append(elstring, "		<EMS_SERIAL></EMS_SERIAL>\n") );
	EXCCHECK(elstring->Append(elstring, "		<EMS_DATE></EMS_DATE>\n") ); 
	EXCCHECK(elstring->Append(elstring, "		<date_time></date_time>\n") ); 
	EXCCHECK(elstring->Append(elstring, "		<barcode_Right></barcode_Right>\n") ); 
	EXCCHECK(elstring->Append(elstring, "		<barcode_Left></barcode_Left>\n") ); 
...
/* <global> */
	...
	EXCCHECK(elstring->Append(elstring, "	</global>\n") );
#ifndef _NO_TEST
	EXCCHECK(elstring->Append(elstring, "	<testseq_init>\n") );
	EXCCHECK(elstring->Append(elstring, "		<failed>0</failed>\n") ); 
	EXCCHECK(elstring->Append(elstring, "		<duration></duration>\n") ); 
	EXCCHECK(elstring->Append(elstring, "	</testseq_init>\n") );

	EXCCHECK(me->GetTestSeq(me, product_id, 0, &testseq, &testseqSize)); 
/* <test> */
	for(i=0; i<testseqSize; i++)
	{
		step_nb++;
	...
	EXCCHECK(elstring->Append(elstring, "	<testseq_cleanup>\n") );
	EXCCHECK(elstring->Append(elstring, "		<failed>0</failed>\n") ); 
	EXCCHECK(elstring->Append(elstring, "		<duration></duration>\n") ); 
	EXCCHECK(elstring->Append(elstring, "	</testseq_cleanup>\n") );
#endif
