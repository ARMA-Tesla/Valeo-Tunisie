...
static SElExceptionPtr test_TestRead(struct _SDBSTestList* me, int32_t branch_id)
{
	...
	/* select items */
	sprintf(sql, "SELECT ts.product_id,ts.test_nb,ts.name,ts.description,ts.time,ts.user_id,ts.property,ts.test_id " 
				 "FROM valeo.product p INNER JOIN valeo.test_steps ts "
				 "ON p.product_id=ts.product_id "
				 "WHERE p.branch_id=%d "
				 "ORDER BY ts.test_nb, ts.product_id", 
				 branch_id);
	...
	for(index=0;index<count;index++)
	{
		...
		ptest[index].test_nb    = *((long*)podbc->GetFieldValuePtr(podbc, 2));
		strcpy(ptest[index].name, (char*)podbc->GetFieldValuePtr(podbc, 3));
		...
		if(desc_len)
		{
			...
			strcpy(ptest[index].description, (char*)podbc->GetFieldValuePtr(podbc, 4));
		}
		strcpy(ptest[index].time, (char*)podbc->GetFieldValuePtr(podbc, 5));
		ptest[index].user_id	= *((long*)podbc->GetFieldValuePtr(podbc, 6));
		ptest[index].property 	= *((short*)podbc->GetFieldValuePtr(podbc, 7));
		ptest[index].test_id 	= *((unsigned long*)podbc->GetFieldValuePtr(podbc, 8));
...
static SElExceptionPtr test_TestSave(struct _SDBSTestList* me, int32_t branch_id)
{
	...
	if(me->TestSize)
	{
		...
		/* get last test_id */
		EXCCHECK( podbc->ExecSQL( podbc, "SELECT MAX(ts.test_id) FROM valeo.test_steps ts"));
		CHECKERR( podbc->Fetch( podbc, TRUE));
		...
		{	
			...
			sprintf(sql, "REPLACE INTO valeo.test_steps "
						 "(product_id,test_nb,name,description,time,user_id,property,test_id) "
						 "VALUES (%d,%d,'%s',_latin2'%s','%s',%d,%d,%d)", 
			...
			for(; i<me->TestSize; i++)
			{
				...
				sprintf(sql, ",(%d,%d,'%s',_latin2'%s','%s',%d,%d,%d)", 
							 me->Test[i].product_id,
							 me->Test[i].test_nb,
							 me->Test[i].name,			
							 GET_STRING(me->Test[i].description), 
...
static SElExceptionPtr test_TestDelHist(struct _SDBSTestList* me, int32_t branch_id)
{
	...
	sprintf(sql, "DELETE FROM valeo.test_steps "
				 "WHERE (test_steps.property&%d)=0 AND test_steps.product_id IN "
				 "(SELECT p.product_id FROM valeo.product p WHERE p.branch_id=%d)", 
				 PROPERTY_VALID,
				 branch_id); 
	EXCCHECK( podbc->ExecSQL( podbc, sql)); 
	EXCCHECK( podbc->Cancel(podbc));
