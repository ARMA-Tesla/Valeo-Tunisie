...
#include "panel_l43_rear_plugin.h"
#include "panel_l43_rear_common.h"
#include "lang_translation.h"

#define NUMBER_OF_PANELS 	3
#define MODULE_JIG			MODULE_JIG_L43_REAR /* guid.h */
				...
				case PANEL_CONNECT:
					if(value)
					{
						if ( gs_PluginArgs )
						{
							...
							checkErr( IviSwtch_Connect (gs_IviSessions[SW4], "ch12", "com12"));
							checkErr( IviDCPwr_ConfigureVoltageLevel( gs_IviSessions[POWER], "2", 13.5));
						}
				...
				case PANEL_CONNECT_2:
					if(value)
					{
						if ( gs_PluginArgs )
						{
							...
							EXCCHECK( pIvi->GetSession( pIvi, "PSW", &gs_IviSessions[PSW]));
							EXCCHECK( pIvi->GetSession( pIvi, "POWER", &gs_IviSessions[POWER]));
							checkErr( IviSwtch_Connect (gs_IviSessions[SW4], "ch22", "com22"));
							checkErr( IviDCPwr_ConfigureVoltageLevel( gs_IviSessions[POWER], "2", 13.5));
						}
						EXCCHECK( PanelSwtch(SWTCH_CONNECT));
					}
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			if(state==TRUE)
				sprintf(command, "\\HWI\\LCD\\w %d %d %d %d", 
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			{
				case PANEL_PWM_BLUE:
				case PANEL_BLUE_DUTY:
				case PANEL_BLUE_FREQ:
					EXCCHECKCVI( GetCtrlVal(panel, PANEL_PWM_BLUE, &status));
					EXCCHECKCVI( GetCtrlVal(panel, PANEL_BLUE_DUTY, &duty));
					EXCCHECKCVI( GetCtrlVal(panel, PANEL_BLUE_FREQ, &freq));
					id = 1;
					break;
				case PANEL_PWM_LCD:
				case PANEL_LCD_DUTY:
				...
				case PANEL_RED_DUTY:
				case PANEL_RED_FREQ:
					EXCCHECKCVI( GetCtrlVal(panel, PANEL_PWM_RED, &status));
					EXCCHECKCVI( GetCtrlVal(panel, PANEL_RED_DUTY, &duty));
					EXCCHECKCVI( GetCtrlVal(panel, PANEL_RED_FREQ, &freq));
					id = 3;
					break;
			}
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	{
		case EVENT_COMMIT:
			sprintf(command, "\\HWI\\AIO\\i16ar 0");
...
#undef __FUNC__
#define __FUNC__ "{Panel L43 REAR Plugin}::Initialize"
PLUGIN_API SElExceptionPtr PLUGIN_INIT_NAME_CODE( SPluginArgsPtr a_pPluginArgs )
{
	...
	EXCCHECK( LoadPanelExWithLanguage(main_panel, "panel_l43_rear.uir", PANEL, PANELlang, &gs_panel[PANEL])); 
	EXCCHECK( LoadPanelExWithLanguage(gs_panel[PANEL], "panel_l43_rear.uir", MEMORY, MEMORYlang, &gs_panel[MEMORY])); 
	EXCCHECK( LoadPanelExWithLanguage(gs_panel[PANEL], "panel_l43_rear.uir", VLINE, VLINElang, &gs_panel[VLINE])); 
...
#undef __FUNC__
#define __FUNC__ "{Panel L43 REAR Plugin}::Run"
...
#undef __FUNC__
#define __FUNC__ "{Panel L43 REAR Plugin}::Leave"
...
#undef __FUNC__
#define __FUNC__ "{Panel L43 REAR Plugin}::Shutdown"
...
#undef __FUNC__
#define __FUNC__ "{Panel L43 REAR Plugin}::Info"
...
#undef __FUNC__
#define __FUNC__ "{Panel L43 REAR Plugin}::Attribute"
