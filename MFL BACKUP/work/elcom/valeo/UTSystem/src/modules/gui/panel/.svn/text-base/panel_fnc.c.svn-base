...
#ifndef _NO_MENU
	extern SZipPtr		gs_pZip;
	extern HINSTANCE	g_hInstance;
	extern char			g_PanelIcons[];
#endif
...
#ifndef _NO_MENU				
		/* picture */
			if( spanel[i-1].pict!=0)
			{
				EXCCHECKCVI( DiscardBitmap(spanel[i-1].pict));
				spanel[i-1].pict = 0;
			} 
			...
			if ( pparameter )
			{
				...
				{	
					...
					sprintf(name_pict, PANEL_PICT, ptext);
					EXCCHECK( gs_pZip->DecompressFile(gs_pZip, g_PanelIcons, name_pict, name_pict));
					
					if(1==GetFileInfo (name_pict, &size))
					{
						EXCCHECKCVI( GetBitmapFromFile (name_pict, &spanel[i-1].pict));
