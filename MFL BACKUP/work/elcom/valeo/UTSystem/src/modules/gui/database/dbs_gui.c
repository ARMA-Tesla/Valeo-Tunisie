...
/* panels positions */
static int32_t					gv_PanelTop = -100;
static int32_t					gv_PanelLeft = -8;
static int32_t					gv_PanelHeight = 104;
static int32_t					gv_PanelWidth = -108;
      ...
      void *callbackData, int eventData1, int eventData2)
{
	...
	{
		case EVENT_COMMIT:
			if(IDOK==MessageBox((HWND)hWnd, _("Read data from database?\nAll changes will be deleted!"), _("Storno Changes"), MB_OKCANCEL|MB_ICONQUESTION))
			{
				if(PDBS->data_changed)
      ...
      void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
		case EVENT_TIMER_TICK:
			 if(global && PDBS)
			 {
				 SetCtrlAttribute (panel, DBS_DB_INSERT, ATTR_DIMMED, (!PDBS->data_changed && !PDBS->data_changed_line));
				 SetCtrlAttribute (panel, DBS_DB_READ, ATTR_DIMMED, (!PDBS->data_changed && !PDBS->data_changed_line));
			 }  
			 break;
	}
...
SElExceptionPtr ShowPanel(void)
{
	...
	if(!visible)
	{
		...
		EXCCHECKCVI( SetCtrlAttribute(gs_panel[DBS], DBS_TIMER_HRUBCA, ATTR_ENABLED, TRUE));
	}
...
SElExceptionPtr ClosePanel(void)          
{
	...
	if(visible)
	{
		...
		EXCCHECKCVI( SetCtrlAttribute(gs_panel[DBS], DBS_TIMER_HRUBCA, ATTR_ENABLED, FALSE));
...
PLUGIN_API SElExceptionPtr PLUGIN_INIT_NAME_CODE( SPluginArgsPtr a_pPluginArgs )
{
	...
	int      		main_panel;
	SDBSUser      	puser = {0}; 
	int				top_main, left_main, height_main, width_main;  
	...
	EXCCHECK( LoadPanelExWithLanguage(gs_panel[DBS], "database.uir", HIST, HISTlang, &gs_panel[HIST]) );
	...
	EXCCHECK( LoadPanelExWithLanguage(gs_panel[DBS], "database.uir", INFO, INFOlang, &gs_panel[INFO]) ); 
	...
	EXCCHECKCVI(GetPanelAttribute (main_panel, ATTR_TOP, &top_main));
	EXCCHECKCVI(GetPanelAttribute (main_panel, ATTR_LEFT, &left_main));
	EXCCHECKCVI(GetPanelAttribute (main_panel, ATTR_HEIGHT, &height_main));
	EXCCHECKCVI(GetPanelAttribute (main_panel, ATTR_WIDTH, &width_main));
	
	//PANEL RESOLUTION
	EXCCHECKCVI( SetPanelAttribute (gs_panel[DBS], ATTR_TOP, top_main + PANEL_TOP));
	EXCCHECKCVI( SetPanelAttribute (gs_panel[DBS], ATTR_LEFT, left_main + PANEL_LEFT));
	EXCCHECKCVI( SetPanelAttribute (gs_panel[DBS], ATTR_HEIGHT, height_main + PANEL_HEIGHT));
	EXCCHECKCVI( SetPanelAttribute (gs_panel[DBS], ATTR_WIDTH, width_main + PANEL_WIDTH));

	EXCCHECKCVI( GetObjHandleFromActiveXCtrl (gs_panel[DBS], DBS_BKTREE, &global->treeHandle) );
	EXCCHECKCVI( GetObjHandleFromActiveXCtrl (gs_panel[HIST], HIST_DTPICKER, &global->pickerHandle) );
	EXCCHECKCVI( GetObjHandleFromActiveXCtrl (gs_panel[INFO], INFO_DTPICKER_1, &global->hpicDayHandle) );
	EXCCHECKCVI( GetObjHandleFromActiveXCtrl (gs_panel[INFO], INFO_DTPICKER_2, &global->hpicHourHandle) ); 
