...
FCE_TEST(MFL, 30_30)
{
	...
	Sleep (100); // old value =50 modified 05.06.2021
	//SetTestValue(pTID, ((STestParamPtr)pTID)->step, "Log", Log_Lin[station]); 
	...
	//	printf("iFlag_Run_Normal_Mode[station]=%d\r\n",iFlag_Run_Normal_Mode[station]);
		if (iFlag_Run_Normal_Mode[station] == 1)
		{
			//printf("break case\r\n");
			break;
		}
	...
	}while (iFlag_Run_Normal_Mode[station] == 0 && i<2); 
...
/*------------- Copied from 30_30 Run Normal Mode---------*/	
	SLinThreadDataPtr	pthread_Lin_data;
	HANDLE				hThreadLin;
    int station = STATION_30;
	clock_t 					_start_test_clock;
	...
	do
	{
			do{
					...
					Sleep(150);   // old value 80 modified 05.06.2021
			...
			}while (iFlag_Read_Valeo_Sw_Revision_ok[STATION_30] == FALSE  && retest < 2);
	if (iFlag_Read_Valeo_Sw_Revision_ok[STATION_30] == TRUE)
		break;
	else
	{
			/* Stop Loop Thread Write */	 
	...
	}while (iFlag_Read_Valeo_Sw_Revision_ok[STATION_30] ==0 && i<2); 
...
FCE_TEST(MFL, 60_10)
{
	...
	/* Filter bessel */
	Filtre(sdWaveGes[0][MESS_TORQUE], read[0], Enum_BesselFilter, Enum1_Lowpass, 3, 0.45, 0.25, 20, BufferTorqueScrollWheelFiltred_UP, &n);
	...
	/* peak detector */	
	Peaks_Detactor(BufferTorqueScrollWheelFiltred_UP, n, Enum2_Peaks, 3 , (int)ceil( mean), &numPeaks_up, Locations_peaks, &len_Locations_peaks);
	...
	/* valley detector */
	Peaks_Detactor(BufferTorqueScrollWheelFiltred_UP, n, Enum2_Valleys, 3 , (int)ceil( mean), &numValleys_up, Locations_valleys, &len_Locations_valleys);
...
//	
	...
	/* -------------- check Nbr Detents LIN Scroll Wheel DOWN*/
	 ...
	 EXCDELETE(pexception);
	 LIMIT_CHECK_EXT(REAL64, Moyenne_Couple_M1_DOWN,pexception); 
	 EXCDELETE(pexception);
